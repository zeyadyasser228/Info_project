...
struct User_fixed {
  string userID[50], username[100], email[150];
};
#pragma endregion

#pragma region Fixed_length
void write_doctor_fixed_length(User_fixed &user)
{
    ofstream file("Doctor.txt"); // open in append mode to not overwrite
    for (int i = 0; i < 3; i++)
    {
...
void read_doctor_fixed_length(User_fixed &User)
{
    ifstream file("User.txt");
    while (file >> User.userID >>User.username >> User.email)
    {
...
int main (){
  ...
  keyword_method_read(u);
  return 0; 
>>>>>>> edecd1dc81086a406cc0c21eedc1a418044b008b
}